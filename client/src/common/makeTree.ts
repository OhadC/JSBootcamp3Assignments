...
export function makeTree(groups: IClientGroup[], userId: string, filterText?: string): ITreeItem[] {
    groups.forEach(group => {
        group.name = group.name || (group.users[0]._id != userId ? group.users[0].name : group.users[1].name)
    })
    const roots: ITreeItem[] = []
    const groupsMap = _.keyBy(groups, '_id')
    const treeItemsMap: { [key: string]: ITreeItem } = {}

    groups.forEach(group => {
        const treeItem: ITreeItem = treeItemsMap[group._id] = treeItemsMap[group._id] || groupToNode(group)  // Seems legit.
